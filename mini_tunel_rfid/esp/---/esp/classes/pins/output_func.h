...
public:
	void set_outputs()
	{
		int motor_on_time = 300;
		static unsigned long current_motor_on = 0;
		if (!motor)
			current_motor_on = millis();

		static byte last_current_retry = 0;
		if (current_retry_cnt != last_current_retry && wait_ans)
		{
			last_current_retry = current_retry_cnt;
			current_motor_on = millis();
		}
		send_current_retry(motor_on_time, current_motor_on);

		if (!wait_ans)
			waiting_answer = false;

		motor = (millis() - current_motor_on > motor_on_time) && !waiting_answer;
...
private:
	...
	void send_current_retry(int motor_on_time, unsigned long current_motor_on)
	{
		static bool sended = false;

		if (millis() - current_motor_on < motor_on_time)
		{
			sended = false;
			return;
		}
