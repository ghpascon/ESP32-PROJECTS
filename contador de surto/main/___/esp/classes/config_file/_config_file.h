...
private:
	...
	void save_parameter(String parameter)
	{
		...
		else if (parameter.startsWith("tipo:"))
		{
			parameter.replace("tipo:", "");
			tipo = parameter;
		}

		else if (parameter.startsWith("id_cliente:"))
		{
			parameter.replace("id_cliente:", "");
			id_cliente = parameter;
		}

		else if (parameter.startsWith("qtd_raios:"))
		{
			parameter.replace("qtd_raios:", "");
			qtd_raios = parameter.toInt();
		}

		else if (parameter.startsWith("Authorization:"))
		{
			parameter.replace("Authorization:", "");
...
public:
	void save_config()
	{
		...
		writeFile("qtd_raios:" + String(qtd_raios));
		writeFile("nome:" + String(nome));
		writeFile("rede:" + String(rede));
		writeFile("senha:" + String(senha));
		writeFile("url:" + String(url));
		writeFile("tipo:" + String(tipo));
		writeFile("id_cliente:" + String(id_cliente));
		writeFile("Authorization:" + String(Authorization));
	}
	void get_config()
	{
