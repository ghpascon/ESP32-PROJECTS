void config_ant_script()
{
    server.on("/save_parameters", HTTP_POST, [](AsyncWebServerRequest *request)
        ...
        if (debug_mode)
        {
            X714_USB.println("PARAMETROS SALVOS COM SUCESSO:");
            for (int i = 0; i < ant_qtd; i++)
            {
                X714_USB.println("");
                X714_USB.println("ANTENA: " + String(i + 1));
                X714_USB.println(antena[i].active ? "ON" : "OFF");
                X714_USB.println("POTENCIA: " + String(antena[i].power));
                X714_USB.println("RSSI: -" + String(antena[i].rssi));
            }
        }
