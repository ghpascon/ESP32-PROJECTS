...
private:
	void writeFile(String to_write)
	{
		...
		if (!file_config)
		{
			X714_USB.println("#E_SAVE_CONFIG");
			return;
		}
		...
		if (debug_mode)
			X714_USB.println("Escrita concluída: '" + to_write + "' foi adicionado ao arquivo");
	}
	...
	void clearFile()
	{
		...
		{
			if (debug_mode)
				X714_USB.println("Falha ao abrir o arquivo para limpar");
			return;
		}
		...
		if (debug_mode)
			X714_USB.println("Arquivo limpo com sucesso");
	}
...
public:
	...
	void get_config()
	{
		...
		{
			if (debug_mode)
				X714_USB.println("Falha ao abrir o arquivo para leitura");
			return;
		}

		if (debug_mode)
			X714_USB.println("Conteúdo do arquivo:");
