...
public:
	void check_cfg_pin()
	{
		static unsigned long cfg_pin_time = 0;
		input_debounce(config, !digitalRead(cfg_pin), cfg_pin_time);

		const int cfg_time = 3000;
		static unsigned long current_cfg_time = millis();

		if (!config)
			current_cfg_time = millis();

		if (millis() - current_cfg_time < cfg_time)
			return;
