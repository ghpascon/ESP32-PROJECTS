...
public:
    void write_tag_cmd(String cmd)
    {
        if (read_on)
        {
            Serial.println("#E_STOP_READ_TO_WRITE");
            return;
        }
        ...
        {
            Serial.println("");
            Serial.println("CURRENT EPC: " + current_epc);
            Serial.println("NEW EPC: " + new_epc);
            Serial.println("PASSWORD: " + password);
            Serial.println("");
        }

        if (current_epc.length() != tag_size || new_epc.length() != tag_size)
        {
    ...
    void verificar_write()
    {
        ...
        if (!answer_rec)
            return;
        gerar_bytes(current_epc, new_epc, password);
        write_tag_on = false;
    }

    void gerar_bytes(String current_epc, String new_epc, String password)
    {
        ...
        reader_module.write_tag(current_epc_array, new_epc_array, password_array);
    }
};
